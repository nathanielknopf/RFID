...
# If it doesn't, it was written by Michael Konrad
#
# TODO:
...
def setup():
	...
	try:
		...
		print csvFilename + " opened successfully."
	except IOError:
...
def updateMiceFlags(tag, gate, mice, timeEpoch):
	for mouse in mice:
		if mouse.tag == tag:
			if gate == "1":
				mouse.inWheel = False
				...
				mouse.inWheel = True
			mouse.writeInOutLine(timeEpoch)
...
def parseLine():
	...
	if data == '':
		...
		# If the data point is a gate being triggered		
			...
			#print("Hasn't Error'd Yet 1")
			# If this data is still within the current block:
			if timeEpoch < endOfBlock:
				tag = data[1:17]
				#print("Hasn't Error'd Yet 2")
				gate = data[20]
				updateMiceFlags(tag, gate, mice, timeEpoch)
				#print("Hasn't Error'd Yet 3")
			# If it is not
			else:
				# First, write the last line and increase endOfBlock
				writeData(mice)
				endOfBlock += interval
				#print("Hasn't Error'd Yet 4")
				# Then, check to see if more than one block has passed.
				# While there are still empty blocks, write lines.
				# Then, continue as normal.
				while (timeEpoch > endOfBlock):
				...
				tag = data[1:17]
				gate = data[20]
				#print("Hasn't Error'd Yet 5")
				updateMiceFlags(tag, gate, mice, timeEpoch)
				#print("Hasn't Error'd Yet 6")
			# except:
